...
class MealsViewController: UIViewController {
  ...
  @IBAction func postMealButton(_ sender: Any) {
    ...
    DBHelper.shared.editFood(postGula: postValue, timestamp: sampleData ?? Date())
    minuteMinute += 5
    hourHour += 2
    print("timestamp post:")
    print(sampleData as Any)
  ...
  @IBAction func insertDataButton(_ sender: Any) {
    ...
    DBHelper.shared.createFood(timestamp:sampleData ?? Date(), nama: foodName, category: foodCategory, image: imageData, preGula: preValue)
      print("timestamp pre:")
    print(sampleData as Any)
